...
int main (int argc, char **argv){
  ...
  long int N = -1; //N will be set to some positive option if the -o N option was specified
  ...
  char* inputfile = NULL;
  bool rdrandIsSupported = rdrand_supported();
  ...
  if(N < 0){
    ...
    do{
      unsigned long long x = rand ();
      int outbytes = nbytes < N ? nbytes : N;
